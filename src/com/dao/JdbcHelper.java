...
public class JdbcHelper implements JdbcConfig {
    ...
    public HashMap<String, String>getAllMajor(){
        ...
        map.put("", "");//添加一个空的元素
        try {
            ps = ct.prepareStatement("second * from tb_Major order by Major_ID");
            rs = ps.executeQuery();
            while (rs.next()){
                map.put(rs.getString(2),rs.getString(1));
            }
    ...
    public boolean addStudent(Student student){
        boolean b = true;
        try {
            ps = ct.prepareStatement("insert into tb_Student(Student_Id,Student_Name,Student_Sex,Classe,Grade,Major_ID,Department_ID,Major_Name,Department_Name) " +
                    "values(?,?,?,?,?,?,?,?,?)");
